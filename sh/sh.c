...
void sh_init ( struct ac_table *g, int alphabet ) {
...
void sh_addstring ( struct ac_table *g, unsigned int i, unsigned char *string, int m, int alphabet ) {
...
unsigned int search_sh ( int m, unsigned char *text, int n, struct ac_table *table, int *bmBc ) {
	...
	while ( column < n ) {
		...
		//printf("j = %i shifting by %i for character %c\n", bmBc[text[column - j + 1]], text[column - j + 1] );
...
struct ac_table *preproc_sh ( unsigned char **pattern, int m, int p_size, int alphabet ) {
	...
	sh_init ( table, alphabet );

	for ( i = 0; i < p_size; i++ )
		sh_addstring ( table, i, pattern[i], m, alphabet );
